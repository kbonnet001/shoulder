...
def create_gcs_seg_0(model, cylinders, segment_names, muscle_index) :
   ...
   if (len(cylinders) == 2) : 
      ...
      # Q_rot, G_rot, H_rot, T_rot = compute_initial_tangent_points(model, cylinders, q, origin_muscle, insertion_muscle, segment_U_index, segment_V_index, gcs_seg_U_0, gcs_seg_V_0, plot = True)
      G_rot = 0
      
   return segment_U_index, segment_V_index, gcs_seg_U_0, gcs_seg_V_0, G_rot

def compute_segment_length(model, cylinders, q, origin_muscle, insertion_muscle, segment_U_index, segment_V_index, gcs_seg_U_0, gcs_seg_V_0, plot = False, G0_q0 = [0,0,0]) :
   ...
   # # origin_muscle_rot = [ 0.08243663 , 0.04414186 ,-0.01228056]
   ...
   if (len(cylinders) == 0) :
      ...
      segment_length = norm(np.array(origin_muscle) - np.array(insertion_muscle))
      ...
      Q, T, Q_T_inactive, segment_length = single_cylinder_obstacle_set_algorithm(origin_muscle, insertion_muscle, cylinders[0])
      ...
      if plot == True : 
         ...
         plot_one_cylinder_obstacle(origin_muscle, insertion_muscle, cylinders, Q, T, Q_T_inactive)
      ...
      print("cyliner U = ", cylinder_U)
      print("cyliner V = ", cylinder_V)
      
      # cylinder_U_rot = np.dot(matrix_rot_zy, cylinder_U)
      # cylinder_V_rot = np.dot(matrix_rot_zy, cylinder_V)
      
      # Q_rot, G_rot, H_rot, T_rot, Q_G_inactive, H_T_inactive , segment_length  = double_cylinder_obstacle_set_algorithm(origin_muscle_rot, insertion_muscle_rot, cylinder_U_rot,cylinders[0].radius, cylinders[0].side, cylinder_V_rot, cylinders[1].radius, cylinders[1].side, np.dot(np.linalg.inv(cylinder_U_rot), cylinder_V_rot), G0_q0)
      ...
      Q, G, H, T, Q_G_inactive, H_T_inactive , segment_length  = double_cylinder_obstacle_set_algorithm(origin_muscle, insertion_muscle, cylinders[0], cylinders[1], np.dot(np.linalg.inv(cylinder_U), cylinder_V), G0_q0)

      if plot == True : 
         plot_double_cylinder_obstacle(origin_muscle, insertion_muscle, cylinders[0], cylinders[1], Q, G, H, T, Q_G_inactive, H_T_inactive)
...
def test_limit_data_for_learning (muscle_selected, cylinders, model, q_ranges, filename, plot=False) :
   ...
   # Find name of segments and matrix of cylinders  
   segment_names = [model.segment(i).name().to_string() for i in range(model.nbSegment())]
   segment_U_index, segment_V_index, gcs_seg_U_0, gcs_seg_V_0, G0_q0= create_gcs_seg_0(model, cylinders, segment_names, muscle_index) 
   ...
   for i in range (3) :
      for j in range (3) :
         for k in range (3) :
            ...
            segment_length = compute_segment_length(model, cylinders, q, origin_muscle, insertion_muscle, segment_U_index, segment_V_index, gcs_seg_U_0, gcs_seg_V_0, plot, G0_q0)  
