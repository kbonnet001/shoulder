...
def test_model_supervised_learning(filename) : 
    X_tensor, y_tensor = data_preparation_create_tensor(filename)
    ...
    # Définir le modèle
    input_size = len(X_tensor[0])  # Nombre de features en entrée
    ...
    model = Model(input_size, output_size, n_layers, n_nodes, activation, L1_penalty, L2_penalty, use_batch_norm)
    ...
    early_stopping = EarlyStopping(monitor='val_mae', patience=70, min_delta=0.00001, verbose=True)
