...
def create_dataset_from_folder_cross_val(mode, file_path, nb_segment):
    """
    ...
    - file_path : str, path to the CSV files with muscle data.
    ...
    - output_size : int, the size of the output labels (y). 
    """
...
def cross_validation(file_path, Hyperparams, mode, num_folds, nb_segment):
    """
    ...
    Warning : This function probably needs some adjustments and corrections ...

    Args:
    - file_path : str, path to the folder containing CSV files with muscle data.
    ...
    - mean_test_error : float, average test error% across all folds.
    - std_test_error : float, standard deviation of test error% across all folds.
    - mean_test_abs_error : float, average test abs error% across all folds.
    - std_test_abs_error : float, standard deviation of test abs error% across all folds.
    """
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_val_dataset)):
        ...
        fold_test_abs_errors.apprend(test_abs_error)
    ...
    mean_test_error = np.mean([np.min(errors) for errors in fold_test_errors])
    std_test_error = np.std([np.min(errors) for errors in fold_test_errors])
    mean_test_abs_error = np.mean([np.min(abs_errors) for abs_errors in fold_test_abs_errors])
    std_test_abs_error= np.std([np.min(abs_errors) for abs_errors in fold_test_abs_errors])

    # Print the results
    print(f'Mean Test Loss: {mean_test_loss:.6f} ± {std_test_loss:.6f}')
    print(f'Mean Test Acc: {mean_test_acc:.6f} ± {std_test_acc:.6f}')
    print(f'Mean Test Error%: {mean_test_error:.6f} ± {std_test_error:.6f}')
    print(f'Mean Test Abs Error%: {mean_test_abs_error:.6f} ± {std_test_abs_error:.6f}')

    return mean_test_loss, std_test_loss, mean_test_acc, std_test_acc, mean_test_error, std_test_error, \
        mean_test_abs_error, std_test_abs_error
