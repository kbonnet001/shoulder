...
def plot_tensor(model, q_fixed, cylinders, muscle_selected, filename, num_points = 100) : 
   ...
   for q_index in range (len(q_ranges)) : 
      ...
      for i in range(len(dlmt_dq)) :
        if i == 0:
            axs[row, col].plot(qs, [dlmt_dqi_biorbd[i] for dlmt_dqi_biorbd in dlmt_dq_biorbds], marker='^', linestyle='--', color = "silver", markersize=1, label=f"dlmt_dq_biorbd")
        else:
            axs[row, col].plot(qs, [dlmt_dqi_biorbd[i] for dlmt_dqi_biorbd in dlmt_dq_biorbds], marker='^', linestyle='--', color = "silver", markersize=1)
            
            axs[row, col].plot(qs, [dlmt_dqi[i] for dlmt_dqi in dlmt_dqis], marker='o', linestyle='-', markersize=1, label=f"dlmt_dq{i}")
   ...
   fig.suptitle(f'{filename} dlmt_dq\nq_fixed = {q_fixed}', fontweight='bold')
   plt.tight_layout()  
   plt.savefig(f"{filename}_dlmt_dq.png")
   plt.show()
